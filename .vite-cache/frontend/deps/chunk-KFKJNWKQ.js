...
function writeBigVarint(t, e) {
  var r, i;
  if (t >= 0 ? (r = t % 4294967296 | 0, i = t / 4294967296 | 0) : (r = ~(-t % 4294967296), i = ~(-t / 4294967296), 4294967295 ^ r ? r = r + 1 | 0 : (r = 0, i = i + 1 | 0)), t >= 18446744073709552e3 || t < -18446744073709552e3) throw new Error("Given varint doesn't fit into 10 bytes");
...
function readTag(t, e) {
  for (var r = t.readVarint() + t.pos; t.pos < r; ) {
    var i = e._keys[t.readVarint()], n = e._values[t.readVarint()];
    e.properties[i] = n;
  }
}
...
!(function(t) {
  ...
  function h(t2) {
    for (var e2 = new Uint8Array(t2), r2 = new Array(e2.length), i2 = 0; i2 < e2.length; i2++) r2[i2] = String.fromCharCode(e2[i2]);
    return r2.join("");
  }
  function l(t2) {
  ...
  function c() {
    return this.bodyUsed = false, this._initBody = function(t2) {
      if (this._bodyInit = t2, t2) if ("string" == typeof t2) this._bodyText = t2;
      else if (v.blob && Blob.prototype.isPrototypeOf(t2)) this._bodyBlob = t2;
      else if (v.formData && FormData.prototype.isPrototypeOf(t2)) this._bodyFormData = t2;
      else if (v.searchParams && URLSearchParams.prototype.isPrototypeOf(t2)) this._bodyText = t2.toString();
      else if (v.arrayBuffer && v.blob && b(t2)) this._bodyArrayBuffer = l(t2.buffer), this._bodyInit = new Blob([this._bodyArrayBuffer]);
      else {
        if (!v.arrayBuffer || !ArrayBuffer.prototype.isPrototypeOf(t2) && !w(t2)) throw new Error("unsupported BodyInit type");
  ...
  function p(t2, e2) {
    ...
    if (this.credentials = e2.credentials || this.credentials || "omit", !e2.headers && this.headers || (this.headers = new n(e2.headers)), this.method = f(e2.method || this.method || "GET"), this.mode = e2.mode || this.mode || null, this.referrer = null, ("GET" === this.method || "HEAD" === this.method) && r2) throw new TypeError("Body not allowed for GET or HEAD requests");
    this._initBody(r2);
  }
  function d(t2) {
    var e2 = new FormData();
  ...
  function y(t2) {
    var e2 = new n();
    return t2.split(/\r?\n/).forEach(function(t3) {
      var r2 = t3.split(":"), i2 = r2.shift().trim();
      if (i2) {
        var n2 = r2.join(":").trim();
        e2.append(i2, n2);
      }
  ...
  if (!t.fetch) {
    ...
    if (v.arrayBuffer) var g = ["[object Int8Array]", "[object Uint8Array]", "[object Uint8ClampedArray]", "[object Int16Array]", "[object Uint16Array]", "[object Int32Array]", "[object Uint32Array]", "[object Float32Array]", "[object Float64Array]"], b = function(t2) {
    ...
    n.prototype.append = function(t2, i2) {
      t2 = e(t2), i2 = r(i2);
      var n2 = this.map[t2];
      this.map[t2] = n2 ? n2 + "," + i2 : i2;
    ...
    }, n.prototype.set = function(t2, i2) {
      this.map[e(t2)] = r(i2);
    }, n.prototype.forEach = function(t2, e2) {
      var r2 = this;
      for (var i2 in this.map) r2.map.hasOwnProperty(i2) && t2.call(e2, r2.map[i2], i2, r2);
    ...
    m.redirect = function(t2, e2) {
      if (-1 === x.indexOf(e2)) throw new RangeError("Invalid status code");
...
var read = function(t, e, r, i, n) {
  var o, s, a = 8 * n - i - 1, u = (1 << a) - 1, h = u >> 1, l = -7, c = r ? n - 1 : 0, f = r ? -1 : 1, p = t[e + c];
...
var write = function(t, e, r, i, n, o) {
  var s, a, u, h = 8 * o - n - 1, l = (1 << h) - 1, c = l >> 1, f = 23 === n ? Math.pow(2, -24) - Math.pow(2, -77) : 0, p = i ? 0 : o - 1, d = i ? 1 : -1, y = e < 0 || 0 === e && 1 / e < 0 ? 1 : 0;
...
Pbf.prototype = { destroy: function() {
  ...
  for (r = r || this.length; this.pos < r; ) {
    var n = i.readVarint(), o = n >> 3, s = i.pos;
  ...
  var r = this, i = readPackedEnd(this);
  ...
  var e = 7 & t;
  if (e === Pbf.Varint) for (; this.buf[this.pos++] > 127; ) ;
...
Point$1.prototype = { clone: function() {
  ...
  var e = Math.cos(t), r = Math.sin(t), i = e * this.x - r * this.y, n = r * this.x + e * this.y;
...
VectorTileFeature$2.types = ["Unknown", "Point", "LineString", "Polygon"], VectorTileFeature$2.prototype.loadGeometry = function() {
  ...
  for (var e, r = t.readVarint() + t.pos, i = 1, n = 0, o = 0, s = 0, a = []; t.pos < r; ) {
    ...
    if (n--, 1 === i || 2 === i) o += t.readSVarint(), s += t.readSVarint(), 1 === i && (e && a.push(e), e = []), e.push(new Point(o, s));
    else {
  ...
  for (var e = t.readVarint() + t.pos, r = 1, i = 0, n = 0, o = 0, s = 1 / 0, a = -1 / 0, u = 1 / 0, h = -1 / 0; t.pos < e; ) {
    ...
    if (i--, 1 === r || 2 === r) n += t.readSVarint(), o += t.readSVarint(), n < s && (s = n), n > a && (a = n), o < u && (u = o), o > h && (h = o);
  ...
  function i(t2) {
    for (var e2 = 0; e2 < t2.length; e2++) {
      var r2 = t2[e2], i2 = 180 - 360 * (r2.y + u) / s;
      t2[e2] = [360 * (r2.x + a) / s - 180, 360 / Math.PI * Math.atan(Math.exp(i2 * Math.PI / 180)) - 90];
    }
  }
...
L.SVG.Tile = L.SVG.extend({ initialize: function(t, e, r) {
  ...
  var e = t._path = L.SVG.create("image"), r = t.options.icon, i = r.options, n = L.point(i.iconSize), o = i.iconAnchor || n && n.divideBy(2, true), s = t._point.subtract(o);
...
var PointSymbolizer = L.CircleMarker.extend({ includes: Symbolizer.prototype, statics: { iconCache: {} }, initialize: function(t, e) {
  ...
  if (t) {
    var e = L.point(t.options.iconSize), r = t.options.iconAnchor || e && e.divideBy(2, true), i = this._point.subtract(r);
...
L.VectorGrid = L.GridLayer.extend({ options: { rendererFactory: L.svg.tile, vectorTileLayerStyles: {}, interactive: false }, initialize: function(t) {
  ...
  var r = this.options.getFeatureId, i = this.getTileSize(), n = this.options.rendererFactory(t, i, this.options), o = this._getVectorTilePromise(t);
  return r && (this._vectorTiles[this._tileCoordsToKey(t)] = n, n._features = {}), o.then(function(i2) {
    for (var o2 in i2.layers) {
      this._dataLayerNames[o2] = true;
      for (var s = i2.layers[o2], a = this.getTileSize().divideBy(s.extent), u = this.options.vectorTileLayerStyles[o2] || L.Path.prototype.options, h = 0; h < s.features.length; h++) {
  ...
  for (var r in this._vectorTiles) {
    var i = this._vectorTiles[r], n = i._features, o = n[t];
    if (o) {
      var s = o.feature, a = e;
  ...
  for (var e in this._vectorTiles) {
    var r = this._vectorTiles[e], i = r._features, n = i[t];
    if (n) {
  ...
  return fetch(i, this.options.fetchOptions).then(function(t2) {
    ...
    for (var e2 in t2.layers) {
      for (var r2 = [], i2 = 0; i2 < t2.layers[e2].length; i2++) {
        var n = t2.layers[e2].feature(i2);
...
L.VectorGrid.Slicer = L.VectorGrid.extend({ options: { vectorTileLayerName: "sliced", extent: 4096, maxZoom: 14 }, initialize: function(t, e) {
  ...
  var e = this, r = new Promise(function(r2) {
    e._worker.addEventListener("message", function i(n) {
      n.data.coords && n.data.coords.x === t.x && n.data.coords.y === t.y && n.data.coords.z === t.z && (r2(n.data), e._worker.removeEventListener("message", i));
    });
  });
  ...
  var e, r, i = this._map.mouseEventToLayerPoint(t).subtract(this.getOffset());
  ...
  if (this._drawing) {
    var e = t.options.icon, r = e.options, i = L.point(r.iconSize), n = r.iconAnchor || i && i.divideBy(2, true), o = t._point.subtract(n), s = this._ctx, a = t._getImage();
...
//# sourceMappingURL=chunk-KFKJNWKQ.js.map
