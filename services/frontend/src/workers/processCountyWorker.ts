// Worker: process county rows into structured school records off the main thread
self.addEventListener('message', (ev: MessageEvent) => {
  ...
  function buildYearlyStudentLookup(rows: any[]) {
    ...
    for (const k of Object.keys(lookup)) lookup[k].sort((a,b)=>a.year-b.year)
    return lookup
  }
  ...
  function buildStudentCompositions(schoolId: string, lookup: any) {
    ...
    return [...years].sort((a,b)=>a-b).map((year)=>{
      ...
      return {
        ...
        bands: bandEntries.map((band) => ({ id: String(band.band_id), label: String(band.band_label), category: String(band.category), totalStudents: Number(band.total_students), maleStudents: band.male_students == null ? undefined : Number(band.male_students), femaleStudents: band.female_students == null ? undefined : Number(band.female_students) }))
      }
    })
  }
  ...
  try {
    ...
    const schoolsByTown = Object.create(null)
