...
/** Serve backend/dist as /data/ in dev and copy it to frontend/dist/data in build. */
function backendDataPlugin(): Plugin {
  let hotReloadTimer: NodeJS.Timeout | null = null
  let isBackendHotReloadReady = false

  const requestDataReload = (server: ViteDevServer, filePath: string) => {
    if (!isBackendDataAsset(filePath)) return
    ...
    hotReloadTimer = setTimeout(() => {
      server.config.logger.info(`backend data changed: ${path.relative(backendDataDir, filePath)}`)
  ...
  return {
    ...
    configureServer(server) {
      server.watcher.add(backendDataDir)
      setTimeout(() => {
        isBackendHotReloadReady = true
      }, 500)
      server.watcher.on('add', (filePath) => {
        if (isBackendHotReloadReady) requestDataReload(server, filePath)
      })
      server.watcher.on('change', (filePath) => requestDataReload(server, filePath))
      server.watcher.on('unlink', (filePath) => requestDataReload(server, filePath))
...
function isBackendDataAsset(filePath: string) {
  const relativePath = path.relative(backendDataDir, filePath)
  if (relativePath.startsWith('..') || path.isAbsolute(relativePath)) return false
  return /\.(json|sqlite|pbf)$/i.test(relativePath)
}
