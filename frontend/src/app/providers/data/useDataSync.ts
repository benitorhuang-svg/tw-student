...
export function useDataSync(deps: SyncDeps) {
  ...
  useEffect(() => {
    ...
    loadDataManifest({ forceRefresh: true })
      ...
      .catch(() => {
        // The returned manifest falls back to local data until remote loading succeeds.
      })
  ...
  return {
    remoteManifest: remoteManifest ?? deps.localManifest,
    refreshSummary,
    isRefreshingData,
    refreshStatus,
    refreshData,
