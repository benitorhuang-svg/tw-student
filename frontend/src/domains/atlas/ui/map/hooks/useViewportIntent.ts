...
export function useViewportIntent(
  ...
  const requestedZoomValue = toFiniteZoom(requestedZoom)
  ...
  return useCallback((): ViewportIntent => {
    ...
    const isZoomCloseToCurrent = requestedZoomValue != null && Math.abs(requestedZoomValue - currentZoom) < 0.05
    const effectiveZoomId = (requestedZoomValue == null || isZoomCloseToCurrent) ? 'auto' : requestedZoomValue.toFixed(1)

    if (selectedSchoolPoint) {
        const zoom = requestedZoomValue ?? pendingInitialZoom ?? MAP_FOCUS_SCHOOL_ZOOM
    ...
    if (activeTownshipId && townshipBoundaries) {
      ...
      if (townshipFeature) {
        ...
        if (bounds.isValid()) {
          ...
          const zoom = requestedZoomValue ?? currentZoom ?? MAP_TOWNSHIP_FOCUS_ZOOM
