...
function getDeviceMemory() {
  try { return (navigator as NavigatorWithDeviceMemory).deviceMemory || 4 } catch { return 4 }
}
...
export async function loadCountyDetail(detailFile: string, countyId?: string) {
  ...
  const nextRequest = (async () => {
    ...
    try {
      ...
      try {
        recordResourceLoad({ source: 'sqlite', resourceKey: detailFile, bytes: 0, countyDetailId: resolvedCountyId })
      } catch {
        // Resource load observation is best-effort.
      }
      return detail
    } finally {
      releaseDetailSlot()
