import { ACADEMIC_YEARS, buildSummaryBucketKey, type CountyRecord, type CountySummaryRecord, type EducationLevelFilter } from '../data/educationData'
import type {
...
export function getTownshipSummaries(county: CountySummaryRecord, filters: DashboardFilters): RankingSummary[] {
  return county.towns
    .map((township) => {
      const matchesSearch = !filters.searchText.trim() || [county.name, township.name].some((value) => value.toLocaleLowerCase().includes(filters.searchText.trim().toLocaleLowerCase()))
